 ...
 * actually proves the YAML is well-formed (a naive string-interpolation bug would still contain
 * the right substrings while producing broken YAML). */
...
describe('renderArchivedChangeDesignPage / renderArchivedChangeTasksPage', () => {
  ...
  it('render design.md/tasks.md verbatim on their own dedicated route, each nested under the change route', () => {
    const repo = buildRepoContent();
    const change = buildArchivedChange({
      designMarkdown: '## Context\n\nUnique design text.',
      tasksMarkdown: '## 1. Do the thing\n\nUnique tasks text.'
    });
    ...
    expect(designPage?.content).toContain('## Context');
    ...
    expect(tasksPage?.content).toContain('## 1. Do the thing');
...
describe('renderArchivedChangeDeltaPage', () => {
  it('renders one page per touched capability, the delta markdown verbatim, and a link onward to its current spec page', () => {
    ...
    const page = renderArchivedChangeDeltaPage(repo, change, delta);
    ...
    expect(page.content).toContain('### Requirement: Foo');
...
describe('renderHomePage', () => {
  it('uses rspress real home-page frontmatter (pageType: home + hero + features)', () => {
...
describe('renderRepoIndexPage', () => {
  it('is the README, verbatim, and nothing else', () => {
    ...
    // Capabilities/archived changes are reachable via the sidebar, not duplicated here.
...
describe('repoSidebarEntries', () => {
  it('always includes an Introduction item pointing at the repo index route', () => {
    const result = buildRegistrySyncResult();
    const sidebar = repoSidebarEntries(result);

    expect(sidebar['/lhx-space/yjs-docs']).toEqual([
      {text: 'Introduction', link: '/lhx-space/yjs-docs'}
    ]);
    // Same array registered under both the bare and trailing-slash keys.
  ...
  it('adds a Specs group and a Changes group (newest first) only when non-empty', () => {
    ...
    // biome-ignore lint/style/noNonNullAssertion: this key is always populated by repoSidebarEntries
    const items = repoSidebarEntries(result)['/lhx-space/yjs-docs']!;
    ...
    // Newest first, so a repo's sidebar reads like a changelog. Since neither change here has a
    // design/tasks/delta, each is a plain link, not a group.
  ...
  it("a differently-prefixed repo's bare key never falsely matches this repo's nested pages", () => {
    // e.g. a registered `yjs-docs` repo must not have its sidebar shown on `yjs-docs-extra`'s
    // pages just because `/lhx-space/yjs-docs` is a string-prefix of
    // `/lhx-space/yjs-docs-extra/specs/x` — rspress picks the *longest* matching key, and
    // `yjs-docs-extra` registers its own (longer) keys, so this never actually collides in
    // practice; this test just documents/locks in that both repos get distinct key sets.
    const yjsDocs = repoSidebarEntries(
      buildRegistrySyncResult({content: buildRepoContent({repo: 'yjs-docs'})})
    );
    const yjsDocsExtra = repoSidebarEntries(
      buildRegistrySyncResult({content: buildRepoContent({repo: 'yjs-docs-extra'})})
    );

    expect(Object.keys(yjsDocs)).toEqual(['/lhx-space/yjs-docs', '/lhx-space/yjs-docs/']);
    expect(Object.keys(yjsDocsExtra)).toEqual([
      '/lhx-space/yjs-docs-extra',
      '/lhx-space/yjs-docs-extra/'
    ]);
  });
});
