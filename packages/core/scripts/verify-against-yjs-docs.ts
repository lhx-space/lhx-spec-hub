...
if (!existsSync(openspecDir)) {
  console.error(
    `yjs-docs/openspec not found at ${openspecDir} — skipping (this script only ` +
      'works when yjs-docs is cloned as a sibling directory of lhx-spec-hub).'
  );
  process.exit(0);
}
