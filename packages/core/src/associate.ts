 ...
 * directory exists (`ArchivedChange.touchedCapabilities`), ordered by `archivedDate` ascending
 * (oldest first — a "history" reads naturally in chronological order). Purely structural — no
 * markdown parsing/NLP, see spec-sync-engine spec.md "capability 关联历史变更". */
export function computeRelatedChanges(
  capabilitySlug: string,
  archivedChanges: ArchivedChange[]
): ArchivedChangeRef[] {
  return archivedChanges
    .filter(change => change.touchedCapabilities.includes(capabilitySlug))
